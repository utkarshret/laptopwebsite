...
const userSchema = new mongoose.Schema({
    ...
    cart: [
     {
        type: mongoose.Schema.ObjectId, //here we have specified the type of the object and referencing always take place using id
        ref: 'Product', //here we are giving reference to the object whose id we are referring to
     } 
    ],
    TotalPrice: {
        type: Number,
...
userSchema.pre(/^find/, function(next){
    console.log("hii");
    this.populate({
        path: 'cart', //here we are specifying the path where we want to populate the data
        select: '-_v' //this key value pair depicts that we dont want _v to be selected and displayed in the final output
        //too many populates in a backend decrease performance of the application 
    }); //populate the cart field i.e all the references are converted to actual real data and shown in the request
    next();
